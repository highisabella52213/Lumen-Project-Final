...
globalThis.__LUMEN_TEST_FETCH__=async (url,options={})=>{calls.push({url:String(url),options});const u=String(url);const body=options.body?JSON.parse(options.body):{};
...
assert.equal(vars.LUMEN_GITHUB_TOKEN,GH);assert.equal(vars.LUMEN_RAILWAY_TOKEN,RW);assert.equal(vars.LUMEN_CREDENTIAL_SOURCE,'installer');assert.equal(vars.LUMEN_REQUIRE_PERSISTENT_STORAGE,'1');
const html=mod.__test.htmlResponse();assert.match(html.headers.get('content-security-policy'),/frame-ancestors 'none'/);assert.match(html.headers.get('content-security-policy'),/fonts.googleapis.com/);assert.match(html.headers.get('cache-control'),/no-store/);const text=await html.text();assert.equal((text.match(/<input /g)||[]).length,2);assert.ok(text.includes('Public Lumen installer')&&text.includes('HTTP proxy'));
const decoded=mod.__test.decodeChunked(Buffer.from('4\r\ntest\r\n5\r\n-data\r\n0\r\n\r\n'));assert.equal(decoded.toString(),'test-data');
const serverSource=(await import('node:fs/promises')).readFile(new URL('../cloudflare-installer/worker.js',import.meta.url),'utf8');const source=await serverSource;const preHtml=source.split('const INSTALLER_HTML =')[0];assert.ok(preHtml.includes('176.111.37.216')&&preHtml.includes('rejectUnauthorized: true'));assert.ok(!preHtml.includes('return await fetch(url'));
const serialized=JSON.stringify(result);assert.ok(!serialized.includes(GH)&&!serialized.includes(RW));delete globalThis.__LUMEN_TEST_FETCH__;
console.log('worker v20: proxy-enforced=OK TLS=OK github=OK railway=OK volume=OK credentials-lock=OK redaction=OK csp=OK');
