...
هر کانفیگ مستقل می‌تواند Direct، یک پروکسی مدیریت‌شده از مخزن، یا پروکسی دلخواه داشته باشد. مخزن خصوصی با AWS Signature V4 از `s3./www-32k-org-021` خوانده می‌شود و هر ۲ ساعت به‌روزرسانی می‌شود. مخزن HTTP/HTTPS/SOCKS5 را می‌پذیرد و Endpointهای مدیریت‌شده به مرورگر ارسال نمی‌شوند؛ کاربر فقط پرچم، کشور، نوع و درصد سلامت را می‌بیند. در v16 مسیر اتصال cache-only است، HTTPSهای رایج هر دو حالت CONNECT و TLS-to-proxy را پشتیبانی می‌کنند و پروکسی بی‌پاسخ با first-byte watchdog به Direct برمی‌گردد تا کانفیگ روی `-1` نماند.
